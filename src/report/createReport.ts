...
function createNarrative(findings: Finding[], detectedStack: DetectedStack): string {
  ...
  return `Static repository scan detected ${detectedStack.all.join(", ")} with ${detectedStack.confidence} confidence. ${warningCount} warning(s) and ${skippedCount} skipped check(s) were reported. Runtime, security secret, API, UI, and AI checks are not part of TODO #2.`;
}
